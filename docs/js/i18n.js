...
const I18n = (() => {
  ...
  const STRINGS = {
    en: {
      ...
      'results.notLeaf': 'This image doesn’t look like a close-up photo of a leaf, so the diagnosis below is unreliable. Photograph a single leaf filling most of the frame — the model only knows <strong>tomato, corn, soybean, and cabbage</strong> leaves.',
      'results.lowConfidence': 'The model isn’t confident about this image. Make sure the photo shows a single leaf, well lit and in focus — and note the model only knows <strong>tomato, corn, soybean, and cabbage</strong> leaves.',
    ...
    es: {
      ...
      'results.notLeaf': 'Esta imagen no parece una foto de cerca de una hoja, así que el diagnóstico no es fiable. Fotografía una sola hoja que llene el encuadre — el modelo solo conoce hojas de <strong>tomate, maíz, soja y col</strong>.',
      'results.lowConfidence': 'El modelo no está seguro de esta imagen. Asegúrate de que la foto muestre una sola hoja, bien iluminada y enfocada — y ten en cuenta que el modelo solo conoce hojas de <strong>tomate, maíz, soja y col</strong>.',
